...
router.get("/",validateToken, async (req: CustomRequest, res: Response) => {
  try { 
    ...
    if (adminCheck == false) { // if user is not an admin fetch only their own cards
      const cards = await Card.find({ userId }) // get cards which have the users id
      res.json(cards) // send cards as JSON 
    } else { // if user is an admin
      const cards = await Card.find() // get all cards in database
      res.json(cards) // send cards as JSON
    }
  } catch (error) {
...
router.get("/:columnId",validateToken, async (req: CustomRequest, res: Response) => {
  try {
    const userId = req.user?.id // extract user id from request
    if (!userId) return res.status(401).json({ message: "unauthorized" }) // check if user exists
    const columnId = req.params.columnId // extract column id from request
    const cards = await Card.find({ columnId, userId }) // fetch all cards that belong to the user and column
...
router.put("/:id",validateToken, async (req: Request, res: Response) => {
  try {
    const updatedCard = await Card.findByIdAndUpdate( // attempt to find a card by its id and update its column id
      req.params.id,
      { columnId: req.body.columnId },
      { new: true } // this makes sure the response will contain the updated card after the change
    )
...
router.delete("/:id",validateToken, async (req: Request, res: Response) => {
  try {
    await Card.findByIdAndDelete(req.params.id) // attempt to find a card by its id and delete it 
